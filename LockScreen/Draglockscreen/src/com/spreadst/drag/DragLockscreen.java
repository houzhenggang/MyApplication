...
public class DragLockscreen extends AbsLockScreen implements
    ...
    private boolean mProgressStopped;
...
//        simManager = SimManager.get(mContext);
    ...
    private void updateProgress() {
            ...
            mProRunnable = new Runnable() {
                public void run() {
                    ...
                    try {
                        ...
                        // Log.d(PACKAGE_NAME, "position= " + position);
                        if (MMUSICSERVICE == null) return;
                        ...
                        Log.d(PACKAGE_NAME, "lastPosition: " + lastPosition + " position: "
                                + position + " mProgressStopped: " + mProgressStopped+" "+DragLockscreen.this);
                        if (!mProgressStopped) {
                            if (lastPosition > position) {
                                /* SPRD: Modify 20140225 Spreadst of Bug 278256 trying to use a recycled bitmap when play music for draglockscreen @{ */
                                mProcesssShowHandle.removeMessages(
                                        MESSAGE_MUSIC_PLAY);
                                /* @} */
                                mProcesssShowHandle.sendEmptyMessageDelayed(
                                        MESSAGE_MUSIC_PLAY, 1000);
                            } else {
                                mProcesssShowHandle.postDelayed(mProRunnable, 1000);
                            }
                            // Log.d(PACKAGE_NAME, "set value position: " +
                            // position);
        ...
        mProcesssShowHandle.removeCallbacks(mProRunnable);
        mProRunnable.run();
    }
    ...
    private void setCarrierTextColor(int phoneId) {
        int color = 0;
//        if (simManager != null) {
//            color = simManager.getColor(phoneId);
//        }
        if (color < 0) {
            mCarrierViews[phoneId].setTextColor(color);
        }
    }

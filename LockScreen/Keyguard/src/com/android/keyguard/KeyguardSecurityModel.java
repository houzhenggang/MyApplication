...
public class KeyguardSecurityModel {
    ...
    SecurityMode getSecurityMode() {
        ...
        if (!isAirplaneMode) {
            ...
            for (int i = 0; i < phoneCount; i++) {
                ...
                if (simState[i] == IccCardConstants.State.PIN_REQUIRED && !updateMonitor.getSimPinUnlockCancel(i)) {
                    return getSimPINSecurityMode(i);
                } else if (simState[i] == IccCardConstants.State.PUK_REQUIRED && !updateMonitor.getSimPukUnlockCancel(i)) {
                    return getSimPUKSecurityMode(i);
                }
                /* @} */
            }
        }
    ...
    /* SPRD: Modify 20140110 Spreadst of Bug 256008 add cancel for sim pin and puk @{ */
    SecurityMode getSimPINSecurityMode(int subscription) {
        SecurityMode mode = SecurityMode.None;
        switch (subscription) {
            case 0:
                mode = SecurityMode.SimPin;
                break;
            case 1:
                mode = SecurityMode.Sim2Pin;
                break;
        }
        return mode;
    }

    SecurityMode getSimPUKSecurityMode(int subscription) {
        SecurityMode mode = SecurityMode.None;
        switch (subscription) {
            case 0:
                mode = SecurityMode.SimPuk;

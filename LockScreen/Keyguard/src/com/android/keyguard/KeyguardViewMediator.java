...
public class KeyguardViewMediator {
    ...
    final static boolean DEBUG = Debug.isDebug();
    ...
    public KeyguardViewMediator(Context context, LockPatternUtils lockPatternUtils) {
		...
		if(YLUNLOCK_SUPPORT){
			...
			filter.addAction("com.yunlan.unlock.ACTION_INITRES");
        ...
        KeyguardUpdateMonitor.getInstance(mContext).setShowing(mShowing);

        WindowManager wm = (WindowManager)context.getSystemService(Context.WINDOW_SERVICE);

        mKeyguardViewManager = new KeyguardViewManager(context, wm, mViewMediatorCallback,
                mLockPatternUtils);
        /* @{ SPRD: fix bug300638 add a Method to release mLockScreenWallpaper*/
        mWpm = WallpaperManager.getInstance(mContext);
        /* @}*/
    ...
    public void handleDismiss() {
        // Modify for Bug 280335
        Log.d(TAG, "handleDismiss mShowing=" + mShowing + "mHidden=" + mHidden);
        if (mShowing /** && !mHidden */) {
    ...
    private void showLocked(Bundle options) {
        ...
        if (android.os.Debug.isDebugOptimizing()) Log.v(TAG, " system_uptime: " + SystemClock.elapsedRealtime());
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            else if("com.yunlan.unlock.ACTION_INITRES".equals(intent.getAction())){
            	boolean isSystem = intent.getBooleanExtra("isSystem", false);
            	String filePathString = intent.getStringExtra("filePath");
            	String pkgName = intent.getStringExtra("pkgName");
            	ArrayList<String> list = intent.getStringArrayListExtra("bgList");
            	
            	if (DEBUG) Log.d(TAG,"-----yunlan-----isSystem=" + isSystem + " filePathString="+ 
            			filePathString + "pkgName="+pkgName + "  pkgName="+pkgName.toString());
    			if(mKeyguardViewManager != null)
    				mKeyguardViewManager.preInitRes(isSystem, filePathString, pkgName, list);
    			Intent it = new Intent("com.yunlan.unlock.ACTION_INITRES_FINISH");
...
//--------------- yunlan begin ----------
			...
			if(KeyguardViewManager.isYunlanOpen && locked==false && YLUNLOCK_SUPPORT){
						 mLockSoundStreamId = mLockSounds.play(whichSound,
                    0, 0, 1/*priortiy*/, 0/*loop*/, 1.0f/*rate*/);
			}else{ 
//--------------- yunlan end ------------- 					
            mLockSoundStreamId = mLockSounds.play(whichSound,
 ...
 //----------------yunlan end -------------
    ...
    private void handleHide() {
        synchronized (KeyguardViewMediator.this) {
            ...
            adjustStatusBarLocked();
            /* @{ SPRD: fix bug300638 add a Method to release mLockScreenWallpaper*/
            mWpm.forgetLoadedLockscreenWallpaper();
            /* }@ */
        }
    }

...
public class KeyguardSimPinView extends KeyguardAbsKeyInputView
    ...
    public void resetState() {
        ...
        if(TelephonyManager.isMultiSim()){
            msg = mContext.getResources().getString(com.android.internal.R.string.kg_sim_pin_instructions_sub,mSubId+1,mRemainTimes);
        } else {
            msg = mContext.getResources().getString(R.string.kg_sim_pin_instructions)+"("+mRemainTimes+")";
        }
        /* @} */
    ...
    protected void onFinishInflate() {
        ...
        /* SPRD: Modify 20140110 Spreadst of Bug 256008 add cancel for sim pin and puk @{ */
        final View cancel = findViewById(R.id.key_sim_pin_cancel);
        if (cancel != null) {
            cancel.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    doHapticKeyClick();
                    KeyguardUpdateMonitor.getInstance(getContext()).updateSimPinUnlockCancel(mSubId);
        ...
        public void run() {
            try {
                ...
                final int[] result = ITelephony.Stub.asInterface(ServiceManager
                        .checkService(TelephonyManager.getServiceName(Context.TELEPHONY_SERVICE, mSubId))).supplyPinReportResult(mPin);
    ...
    private class GetRemainTimes extends Thread {
        @Override
        public void run() {
            Log.d(TAG, "run....");
            mRemainTimes = TelephonyManager.getDefault(mSubId).getRemainTimes(
                    TelephonyManager.UNLOCK_PIN);
            Log.d(TAG, "over..." + mRemainTimes);
            mHandler.post(new Runnable() {
                public void run() {
                    Log.d(TAG, "get RemainTimes ok " + mRemainTimes);
                    resetState();
                }
            });
        }
    ...
    public void setSubId(int subId) {
        mSubId = subId;
        Log.d(TAG, "start get RemainTimes");
        new GetRemainTimes().start();
    }

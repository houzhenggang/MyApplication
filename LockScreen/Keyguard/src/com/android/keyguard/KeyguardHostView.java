...
import com.sprd.keyguard.IKeguardLockView;
...
public class KeyguardHostView extends KeyguardViewBase {
    ...
    // SPRD: Modify 20131211 Spreadst of Bug 250136 print log info
...
//------------------------yunlan begin--------------------------
	...
	private  View mYunlanLockScreen=null;
	private boolean isYunlanEnable = false;
//------------------------yunlan end  --------------------------
    ...
    /* SPRD: Modify 20131128 Spreadst of Bug 245693 UUI lockscreen feature @{ */
    ...
    private final String SYSTEM_SETTINGS_LOCKSTYLE = "system.settings.lockstyle";
    /* @} */

    /* SPRD: Modify 20140107 Spreadst of Bug 257891 UUILockScreen can not emergencycall @{ */
    private final String ACTION_EMERGENCY_DIAL = "com.android.phone.EmergencyDialer.DIAL";
    ...
    public KeyguardHostView(Context context, AttributeSet attrs) {
        ...
        // SPRD: Modify 20131228 Spreadst of Bug 261394 UUI lockscreen policy change
    ...
    protected void onFinishInflate() {
        ...
        challenge = mSlidingChallengeLayout != null ? mSlidingChallengeLayout :
                mMultiPaneChallengeLayout;
...
//------------------------yunlan begin--------------------------
        if(YLUNLOCK_SUPPORT){
			if (DEBUG)Log.d(TAG,"-----yunlan------updateAndAddWidgets creatAndAddYunlanUnlockView");
        	creatAndAddYunlanUnlockView();
		}
//------------------------yunlan end  --------------------------
    }
  ...
  */
    private void updateSecurityView(View view,SecurityMode securityMode) {
        if (view instanceof KeyguardSecurityView) {
             ...
             if (securityMode != null && ksv instanceof IKeguardLockView) {
                 IKeguardLockView kview = (IKeguardLockView)ksv;
    ...
    private KeyguardSecurityCallback mCallback = new KeyguardSecurityCallback() {
        ...
        /* SPRD: Modify 20131127 Spreadst of Bug 244716 support multi-card for Pin and Puk @{ */
        @Override
    ...
    void showPrimarySecurityScreen(boolean turningOff) {
        ...
        /* SPRD: Modify 20131230 Spreadst of Bug 262132 modify UUI lockscreen policy @{ */
        // SPRD: Modify 20140110 Spreadst of Bug 267214 show uui lock when lock is disable
        if (mUniverseui_support && !mLockPatternUtils.isLockScreenDisabled() && KeyguardViewManager.IsShowUUILock() && (0 == getUUILockId())) {
    ...
    private void showNextSecurityScreenOrFinish(boolean authenticated) {
        ...
        if (lockView != null) {
            lockView.onPause();
            lockView.setKeyguardCallback(mNullCallback);
            removeView((View)lockView);
            KeyguardViewManager.setShowUUILock(false);
            lockView = null;
            if (!finish) {
                ((View)challenge).setVisibility(View.VISIBLE);
            }
        }
        /* @} */
        ...
        if (finish) {
            ...
            /* SPRD: Modify 20140110 Spreadst of Bug 264519 sim1 pin unlock UI is different sim2 @{
    ...
    private void showSecurityScreen(SecurityMode securityMode) {
		...
		//------------------------yunlan begin--------------------------
		if(YLUNLOCK_SUPPORT){
	        if (DEBUG) Log.d(TAG, "-----yunlan------showSecurityScreen    mYunlanLockScreen=" + mYunlanLockScreen);
	        if (mYunlanLockScreen == null && securityMode == KeyguardSecurityModel.SecurityMode.None ) {
	            if (DEBUG)Log.d(TAG,"-----yunlan------showSecurityScreen creatAndAddYunlanUnlockView");
	            creatAndAddYunlanUnlockView();
			}
			if (mYunlanLockScreen != null) {
				if (DEBUG) Log.d(TAG, "-----yunlan------showSecurityScreen    =======1");
				if (securityMode == KeyguardSecurityModel.SecurityMode.None) {
					if (DEBUG) Log.d(TAG, "-----yunlan------showSecurityScreen    =======2");
					mYunlanLockScreen.setVisibility(View.VISIBLE);
					if (mSlidingChallengeLayout != null) {
						if (DEBUG) Log.d(TAG, "-----yunlan------showSecurityScreen    =======3");
						mSlidingChallengeLayout.setVisibility(View.GONE);
					}
					((KeyguardSecurityView)mYunlanLockScreen).onResume(KeyguardSecurityView.VIEW_REVEALED);
						((KeyguardSecurityView)mYunlanLockScreen).setKeyguardCallback(mCallback);
					isYunlanEnable = true;
				} else {
					if (DEBUG) Log.d(TAG, "-----yunlan------showSecurityScreen    =======4");
					isYunlanEnable = false;
					if (mSlidingChallengeLayout != null) {
						if (DEBUG) Log.d(TAG, "-----yunlan------showSecurityScreen    =======5");
						mSlidingChallengeLayout.setVisibility(View.VISIBLE);
					}
					mYunlanLockScreen.setVisibility(View.GONE);
					((KeyguardSecurityView)mYunlanLockScreen).onPause();
						((KeyguardSecurityView)mYunlanLockScreen).setKeyguardCallback(mNullCallback);
				}
			} 
		}
		//------------------------yunlan end  --------------------------
    ...
    public void onScreenTurnedOn() {
		...
		//------------------------yunlan begin--------------------------
		if(YLUNLOCK_SUPPORT){
			if (mYunlanLockScreen != null && isYunlanEnable) {
				((KeyguardSecurityView)mYunlanLockScreen).onResume(KeyguardSecurityView.SCREEN_ON);
			}
		}
		//------------------------yunlan end  --------------------------
    ...
    public void onScreenTurnedOff() {
        ...
        KeyguardUpdateMonitor.getInstance(mContext).setAlternateUnlockEnabled(true);
        // SPRD: Modify 20140110 Spreadst of Bug 256008 add cancel for sim pin and puk
    ...
    public void cleanUp() {
		...
		if(YLUNLOCK_SUPPORT){
			if (mYunlanLockScreen != null && isYunlanEnable) {
				((KeyguardSecurityView)mYunlanLockScreen).onPause();
				((KeyguardSecurityView)mYunlanLockScreen).setKeyguardCallback(mNullCallback);
				((KeyguardViewBase)mYunlanLockScreen).cleanUp();
				this.removeView(mYunlanLockScreen);
			}
		}
		//------------------------yunlan end  --------------------------
    }
    ...
    public boolean handleBackKey() {
        // SPRD: Modify 20140115 Spreadst of Bug 266992 dismiss uui lock when handle back key
        if (mUniverseui_support && KeyguardViewManager.IsShowUUILock()) return false;
    ...
    /* SPRD: Modify 20131230 Spreadst of Bug 262132 modify UUI lockscreen policy @{ */
    private void showUUILockScreen() {
        // SPRD: Modify 20140110 Spreadst of Bug 267214 show uui lock when lock is disable
        if (mUniverseui_support && !mLockPatternUtils.isLockScreenDisabled() && KeyguardViewManager.IsShowUUILock()){
            if (lockView == null) {
                ...
                try {
                    ...
                    ((View)challenge).setVisibility(View.INVISIBLE);
                } catch (Exception e) {
    ...
    private int getUUILockId() {
        return Settings.System.getInt(mContext.getContentResolver(),
                SYSTEM_SETTINGS_LOCKSTYLE, 0);
    }
    /* @} */
     ...
     */
    private void doEmergencyCallAction() {
    ...
    public void onPauseForUUI() {
        if (lockView != null) {
            lockView.onPause();
        }
    }
//------------------------yunlan begin--------------------------
	private void creatAndAddYunlanUnlockView() {
		View curView = null;
		if (curView == null) {
			try {
				Configuration mConfiguration = mContext.getResources().getConfiguration();
				if (DEBUG) Log.d(TAG, "-----yunlan------KeyguardViewManager.isYunlanOpen=" + KeyguardViewManager.isYunlanOpen);
				if(KeyguardViewManager.isYunlanOpen) {
					BitmapDrawable tmpDrawable=null;
				
					tmpDrawable = KeyguardViewManager.initYunlanDrawable();
					
					String curPackageName=getCurrentPkg(mContext);				
					if (DEBUG) Log.d("TAG","KeyguardHostView  savePackageName1"+KeyguardViewManager.savePackageName1+
							"    curPackageName="+curPackageName+"  sType1 = "+KeyguardViewManager.sType1);
					if(!curPackageName.equals(KeyguardViewManager.savePackageName1) && KeyguardViewManager.sType1 == 0){
						tmpDrawable = null;
					}
					
					Settings.System.putString(mContext.getContentResolver(), "yunlan_wallpaper_name", KeyguardViewManager.savePackageName1);

					long old = System.currentTimeMillis();
					curView = new YunlanUnlockView(mContext, mConfiguration, mCallback,
							tmpDrawable, KeyguardViewManager.sType1, mActivityLauncher);
					
					if (DEBUG) Log.d(TAG,"-----yunlan---ffffffuser="+(System.currentTimeMillis()-old));

					mYunlanLockScreen = curView;
					if (DEBUG) Log.d(TAG,"-----yunlan------mCurrentView  success mYunlanLockScreen ="+mYunlanLockScreen);
				}
			} catch (Exception e) {
				e.printStackTrace();
				 if (DEBUG)Log.d(TAG,"-----yunlan------mCurrentView Exception");
				curView = null;
				mYunlanLockScreen = null;
			}  
		}
		if (mYunlanLockScreen != null) {
			this.addView(mYunlanLockScreen, 0);
			mYunlanLockScreen.setVisibility(View.GONE);
		}
	}
	...
	private String getCurrentPkg(Context context){
		String ss = "";	
		Uri CONTENT_URI = Uri.parse("content://com.yunlan.syslockmarket/currentpackage");
		Cursor cursor = null;
		try{
			...
			if(cursor != null) {
				while (cursor.moveToNext()) {
					...
					if (DEBUG) Log.d(TAG,"---xu--- currentpackageName="+ss);
				}
				cursor.close();
			}
		}catch(Exception e){
			if (DEBUG) Log.d(TAG,"--Exception----xu--- currentpackageName="+ss);
		}
		return ss;
	}
	//------------------------yunlan end  --------------------------

...
public class KeyguardUpdateMonitor {
    ...
    private static final int MSG_UNREAD_MESSAGE_COUNT = 330;
    ...
    private static final int MSG_MISSED_CALL_COUNT = 331;

    private static final int MSG_AIR_APLANE_CHANGE = 400;
    private final static String ACTION_MISSED_CALL_COUNT = "com.android.call.MISSED_CALL";
    private final static String EXTRA_MISSED_CALL_KEY ="EXTRA_MISSED_CALL_KEY";
    ...
    private boolean[] mIsSimPinUnlockCancelled;
    private boolean[] mIsSimPukUnlockCancelled;
    /* @} */
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SIM_STATE_CHANGE:
                    /* SPRD: Modify 20140110 Spreadst of Bug 260939 sim pin dialog show forever or unlock to sim2 pin wait a long time @{
                    ...
                    if (state[subscription] != mSimState[subscription]) {
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            // SPRD: Modify 20131231 Spreadst of Bug 258935 show wrong sim info
            ...
            /* @} */
                if (true) {
                    Log.v(TAG, "keyguardmonitor action = " + action + " keyguardmonitor state" +
            ...
            /* SPRD: Modify 20131227 Spreadst of Bug 253124 UUILockScreen not show missedcall and unread message info @{ */
                ...
                int unreadmessagecount = intent.getIntExtra(EXTRA_MESSAGE_COUNT, 0);
                mUnreadMessageCount = unreadmessagecount;
                if (DEBUG) {
                    Log.i(TAG, "ACTION_UNREAD_MESSAGE_COUNT:get num of unread message :"
                            + unreadmessagecount);
                }
                mHandler.sendMessage(mHandler.obtainMessage(MSG_UNREAD_MESSAGE_COUNT,
                        unreadmessagecount,
                        0));
            } else if (action.equals(ACTION_MISSED_CALL_COUNT)) {
                int misscallcount = intent.getIntExtra(EXTRA_MISSED_CALL_KEY, 0);
                mMissedCallCount = misscallcount;
                if (DEBUG) {
                    Log.i(TAG, "ACTION_MISSED_CALL_COUNT:get num of unread message :"
                            + misscallcount);
                }
            ...
            /* @} */
                ...
                mHandler.sendMessage(mHandler.obtainMessage(MSG_AIR_APLANE_CHANGE));
            }
        }
    };
    ...
    private static class SimArgs {
        ...
        static SimArgs fromIntent(Intent intent) {
            ...
            if (!(intent.getAction()).startsWith(TelephonyIntents.ACTION_SIM_STATE_CHANGED)) {
            /* @} */
                throw new IllegalArgumentException("only handles intent ACTION_SIM_STATE_CHANGED");
            }
            String stateExtra = intent.getStringExtra(IccCardConstants.INTENT_KEY_ICC_STATE);
            Log.d(TAG, "  keyguardmonitor stateExtra :" + stateExtra+" id :"+simSubscription +" reson = "+intent
	...
	//------------------------yunlan start--------------------------
    /* package */public static class BatteryStatus {
	//------------------------yunlan start--------------------------
    ...
    private KeyguardUpdateMonitor(Context context) {
        ...
        if (phoneCount > 1) {
            ...
            mSimState[phoneCount - 1] = IccCardConstants.State.UNKNOWN;
        }
        /* @} */

        /* SPRD: Modify 20140110 Spreadst of Bug 256008 add cancel for sim pin and puk @{ */
        mIsSimPinUnlockCancelled = new boolean[phoneCount];
        mIsSimPukUnlockCancelled = new boolean[phoneCount];
        for (int i = 0; i < phoneCount; i++) {
            mIsSimPinUnlockCancelled[i] = false;
            mIsSimPukUnlockCancelled[i] = false;
        }
        /* @} */
        ...
        if (TelephonyManager.isMultiSim()) {
            for (int phoneId = 0; phoneId < TelephonyManager.getPhoneCount(); phoneId++) {
                filter.addAction(TelephonyIntents.ACTION_SIM_STATE_CHANGED + phoneId);
            }
        } else {
            filter.addAction(TelephonyIntents.ACTION_SIM_STATE_CHANGED);
        }
        ...
        if (TelephonyManager.isMultiSim()) {
            for (int phoneId = 0; phoneId < TelephonyManager.getPhoneCount(); phoneId++) {
                filter.addAction(TelephonyIntents.SPN_STRINGS_UPDATED_ACTION + phoneId);
            }
        } else {
    ...
    private void handleSimStateChange(SimArgs simArgs) {
        ...
        if (true) {
            Log.d(TAG, "handleSimStateChange: intentValue = " + simArgs.subscription + " "
                    + "state resolved to " + state[simArgs.subscription].toString());
        }
        /* SPRD: Modify 20140110 Spreadst of Bug 260939 sim pin dialog show forever or unlock to sim2 pin wait a long time @{
        if (state[subscription] != IccCardConstants.State.UNKNOWN && state != mSimState) { */
        if (state[subscription] != IccCardConstants.State.UNKNOWN) {
        /* @} */
    ...
    private void handleAirPlaneChange() {
        boolean airPlaneMode = (Settings.Global.getInt(mContext.getContentResolver(),
                Settings.Global.AIRPLANE_MODE_ON, 0) == 1);
        Log.d(TAG, "airPlaneMode,mAirplaneMode,mShowing =" + airPlaneMode + "," + mAirplaneMode
                + "," + mShowing);
        if (mAirplaneMode != airPlaneMode) {
            mAirplaneMode = airPlaneMode;
            int phoneCount = TelephonyManager.getPhoneCount();
            for (int i = 0; i < phoneCount; i++) {
                if (!airPlaneMode && (mSimState[i] == IccCardConstants.State.PIN_REQUIRED
                        || mSimState[i] == IccCardConstants.State.PUK_REQUIRED) && !mShowing) {
                    KeyguardViewManager.setShowUUILock(false);
                    for (int j = 0; j < mCallbacks.size(); j++) {
                        KeyguardUpdateMonitorCallback cb = mCallbacks.get(j).get();
                        if (cb != null) {
                            cb.onSimStateChanged(mSimState[i], i);
                        }
                    }
                }
            }
    ...
    private void handeleMissedCallCountChanged(int count) {
        if (DEBUG)
            Log.d(TAG, "handeleMissedCallCountChanged() :" + count);
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
    ...
    /* SPRD: Modify 20140110 Spreadst of Bug 256008 add cancel for sim pin and puk @{ */
    public void updateSimPinUnlockCancel(int subscription) {
        mIsSimPinUnlockCancelled[subscription] = true;
    }

    public void updateSimPukUnlockCancel(int subscription) {
        mIsSimPukUnlockCancelled[subscription] = true;
    }

    boolean getSimPinUnlockCancel(int subscription) {
        return mIsSimPinUnlockCancelled[subscription];
    }

    boolean getSimPukUnlockCancel(int subscription) {
        return mIsSimPukUnlockCancelled[subscription];
    }

    public void resetSimUnlockCancel() {
        int phoneCount = TelephonyManager.getPhoneCount();
        for (int i = 0; i < phoneCount; i++) {
            mIsSimPinUnlockCancelled[i] = false;
            mIsSimPukUnlockCancelled[i] = false;
